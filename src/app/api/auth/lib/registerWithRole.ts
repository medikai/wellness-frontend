...
export async function registerWithRole(req: Request, role: 'student'|'coach') {
  ...
  const { error: roleErr } = await supabaseAnon.rpc('set_account_role', { p_role: role });
  if (roleErr) {
    return NextResponse.json({ ok: false, error: roleErr.message }, { status: 400 });
  }

  const isProd = process.env.NODE_ENV === 'production';
  const res = NextResponse.json({ ok: true, profile: upserted, role }, { status: 200 });
  ...
  res.cookies.set('sb-refresh-token', sessionData.session.refresh_token, {
    httpOnly: true, secure: isProd, sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 * 28,
  });
