-- 20251022123000_fix_book_demo_slot.sql
...
begin
  ...
  with params as (
    select
      ...
      extract(epoch from (v_slot_end::time))::int / 60 as em
  ),
  weekly as (
    select car.coach_id
  ...
  eligible as (
    select coach_id from weekly
    union
    select coach_id from excs
  ),
  free as (
    select e.coach_id
    from eligible e
